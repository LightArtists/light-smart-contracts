...
describe("ThreeChiefOfficersWithRoyalties", async function () {
    const ROYALTY_AMOUNT = 700; // basis points
    let ThreeChiefOfficersMock;
    let threeChiefOfficersMock;
    let ceo, cfo, coo, otherAccount;

    before(async function () {
        [ceo, cfo, coo, otherAccount] = await ethers.getSigners();
    });

    describe("initial state", async function () {
        beforeEach(async function () {
            ThreeChiefOfficersMock = await ethers.getContractFactory("ThreeChiefOfficersWithRoyaltiesMock", ceo);
            threeChiefOfficersMock = await ThreeChiefOfficersMock.deploy(cfo.address, ROYALTY_AMOUNT);
        });
        ...
        it("the initial CFO should NOT be empty", async function () {
            expect(await threeChiefOfficersMock.financialOfficer()).to.not.equal(ethers.constants.AddressZero);
        });
    ...
    describe("setting CEO", async function () {
        ...
        it("should not allow random person to set CEO", async function () {
            ...
            await expect(
                threeChiefOfficersMock.connect(otherAccount).setExecutiveOfficer(otherAccount.address)
    ...
    describe("setting CFO", async function () {
        ...
        it("should not allow random person to set CFO", async function () {
            await expect(
                threeChiefOfficersMock.connect(otherAccount).setFinancialOfficer(otherAccount.address)
            ).to.be.revertedWith("NotExecutiveOfficer()");
        });
    });
    ...
    describe("setting COO", async function () {
        ...
        it("should not allow random person to set COO", async function () {
            ...
            await expect(
                threeChiefOfficersMock.connect(otherAccount).setOperatingOfficer(otherAccount.address)
            ).to.be.revertedWith("NotExecutiveOfficer()");
        });
    });
    ...
    describe("performing COO privileged actions", async function () {
        ...
        it("should not allow random person to perform COO privileged actions", async function () {
            await threeChiefOfficersMock.connect(ceo).setOperatingOfficer(coo.address);
            await expect(
                threeChiefOfficersMock.connect(otherAccount).somethingOnlyOperatingOfficerCanDo()
    ...
    describe("performing CFO privileged actions", async function () {
        ...
        it("should allow CFO to perform privileged action", async function () {
            ...
            await threeChiefOfficersMock.connect(cfo).withdrawBalance();
        ...
        it("should not allow random person to perform CFO privileged action", async function () {
            ...
            await expect(threeChiefOfficersMock.connect(otherAccount).withdrawBalance()).to.be.revertedWith(
                "NotFinancialOfficer()"
            );
        });
    });
